...
function materialized(schema, options) {
    ...
    schema.path('path').set(function(v){
    ...
    // --- checkers ------------------------------------------------
    schema.static('sIsRoot', function (elOrId, callback) {
    ...
    schema.static('sIsLeaf', function (elOrId, callback) {
        var id = elOrId._id || elOrId;
        var self = this;
        var promise = new mongoose.Promise;
    ...
    schema.method('isDescendant', function (elOrId, callback) {
        var id = elOrId._id ? elOrId._id : elOrId;
        var self = this;
        var promise = new mongoose.Promise;
        if (callback)
            promise.addBack(callback);
    ...
    schema.static('addChild', function (parent, child, callback) {
        var self = this;
        var promise = new mongoose.Promise;
        if (callback)
            promise.addBack(callback);
